...
test('all release-facing packages and manifests use one version', async () => {
  ...
  assert.deepEqual(versions, Array(versions.length).fill('4.5.4'));
});

test('the package lock agrees with the root and workspace package versions', async () => {
  const lock = await readJson('package-lock.json');

  assert.equal(lock.version, '4.5.4');
  assert.equal(lock.packages[''].version, '4.5.4');
  assert.equal(lock.packages.extension.version, '4.5.4');
  assert.equal(lock.packages['packages/ob-plugin'].version, '4.5.4');
