...
test('all release-facing packages and manifests use one version', async () => {
  ...
  assert.deepEqual(versions, Array(versions.length).fill('4.1.0'));
});
...
test('the package lock agrees with the root and workspace package versions', async () => {
  ...
  assert.equal(lock.packages.extension.version, '4.1.0');
  assert.equal(lock.packages['packages/ob-plugin'].version, '4.1.0');
