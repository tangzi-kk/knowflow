...
| 组件 | 版本 | 源码/产物 | 状态 |
|---|---:|---|---|
| Obsidian `fs-TB` | `4.2.0` | `artifacts/fs-TB-Obsidian-4.2.0.zip` | 当前稳定版；已安装真实 Vault，状态握手与全库预览通过 |
| 浏览器 `KnowFlow` | `4.2.0` | `artifacts/KnowFlow-Browser-4.2.0/`、同名 ZIP | 当前稳定版；构建、版本一致性和通信协议门禁通过 |
...
```text
artifacts/fs-TB-Obsidian-4.2.0.zip
```
...
```text
artifacts/KnowFlow-Browser-4.2.0/
```

也可以先解压 `artifacts/KnowFlow-Browser-4.2.0.zip`。浏览器应选择解压后的目录，不能直接选择 ZIP。
...
```bash
shasum -a 256 -c artifacts/KnowFlow-4.2.0-SHA256SUMS
(cd artifacts && shasum -a 256 -c KnowFlow-4.2.0-PACKAGES-SHA256SUMS)
```
