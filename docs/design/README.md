...
- 4.3 的自动识别把新建/修改事件和全库扫描收口为自动事务；异常项单篇跳过，用户用右键手动修正。
...
- 当前验收边界：[`../validation/4.3-acceptance.md`](../validation/4.3-acceptance.md)
...
- 文件右键菜单只处理当前选中的文档、目录或多选内容；它是自动编码后的异常纠错入口，打开轻量纠错面板。
