...
(() => {
  ...
  // ../../../../Users/changbeifenggongzuoshi/my/Projects/插件设计/sync-plugin/node_modules/js-yaml/dist/js-yaml.mjs
  ...
  async function postFetch(config, req) {
    ...
    const result = await request(config, "POST", "/fetch", withRequestId(req), 12e4);
    assertProposalResponse(result);
    return result;
  }
  async function postClip(config, req) {
    await requireWriteCapability(config, "clip");
    const result = await request(config, "POST", "/clip", withRequestId(req), 3e4);
    assertProposalResponse(result);
    return result;
  }
  function assertProposalResponse(result) {
    if (typeof result.path === "string" && result.path && typeof result.proposalId === "string" && result.proposalId && result.requiresConfirmation === true && result.protocolVersion === PROTOCOL_VERSION) {
      return;
    }
    throw new Error(
  ...
  // ../../../../Users/changbeifenggongzuoshi/my/Projects/插件设计/sync-plugin/node_modules/js-sha3/src/sha3.js
  var require_sha3 = __commonJS({
    "../../../../Users/changbeifenggongzuoshi/my/Projects/\u63D2\u4EF6\u8BBE\u8BA1/sync-plugin/node_modules/js-sha3/src/sha3.js"(exports, module) {
      (function() {
        "use strict";
  ...
  var DEFAULT_GEMINI_WEB_MODEL = "fbb127bbb056c959";
  var LEGACY_GEMINI_WEB_MODELS = /* @__PURE__ */ new Set([
    "56fdd199312815e2",
    "8c46e95b1a07cecc"
  ]);
  var GEMINI_WEB_ALIASES = /* @__PURE__ */ new Map();
  ...
  function modelOrDefault(model, fallback) {
    const value = model?.trim() || "";
    return !value || LEGACY_GEMINI_WEB_MODELS.has(value) ? fallback : value;
  }
  function chatCompletionsEndpoint(baseUrl2) {
    const normalized = baseUrl2.trim().replace(/\/+$/, "");
    if (!normalized) return "";
    return normalized.endsWith("/chat/completions") ? normalized : `${normalized}/chat/completions`;
  }
  function resolveAiRoute(config) {
    ...
    if (config.provider === "gemini-api") {
      const model2 = modelOrDefault(config.model, "gemini-3.6-flash");
      return {
        kind: "gemini-api",
        endpoint: `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model2)}:generateContent`,
        model: model2,
        requiresApiKey: true
      };
    }
    ...
    const defaultBaseUrl = isOpenAi ? "https://api.openai.com/v1" : isDeepSeek ? "https://api.deepseek.com" : "";
  ...
  var WorkflowStore = class {
    ...
    transition(operationId, nextState, payload = {}) {
      return this.mutate((operations) => {
        ...
        if (nextState === "succeeded" || nextState === "awaiting-confirmation") {
          ...
          if (nextState === "awaiting-confirmation" && (!payload.result.proposalId || payload.result.requiresConfirmation !== true)) {
    ...
    async run(kind, requestId, task) {
      ...
      try {
        ...
        await this.transition(
          ...
          result.proposalId && result.requiresConfirmation === true ? "awaiting-confirmation" : "succeeded",
          { result }
        );
        return result;
      } catch (error) {
    ...
    prune(operations) {
      const active = operations.filter((operation) => operation.state === "queued" || operation.state === "running");
      const retained = operations.filter((operation) => operation.state !== "queued" && operation.state !== "running").sort((left, right) => right.updatedAt - left.updatedAt).slice(0, Math.max(0, this.maxOperations - active.length));
  ...
  function normalizeContextScenes(input) {
    ...
    for (const item of rawScenes) {
      if (item && typeof item === "object") {
        const rawId = item.id;
        const fallback = rawId ? fallbackById.get(rawId) : void 0;
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.type === "ai-inline-deepseek-web") {
      ...
      withTimeout(
        ...
        AI_TIMEOUT_MS,
        AI_TIMEOUT_MESSAGE
      ).then((result) => sendResponse({ text: result })).catch((error) => sendResponse({ error: error instanceof Error ? error.message : String(error) }));
      return true;
    }
    ...
    if (message.type === "set-deepseek-token") {
      ...
      setDeepSeekToken(token).then(() => sendResponse({ ok: true, hasToken: isValidToken(token) })).catch((error) => sendResponse({ ok: false, error: error instanceof Error ? error.message : String(error) }));
      return true;
    }
    if (message.type === "get-deepseek-token") {
      getDeepSeekToken().then((token) => sendResponse({ token: token || "", hasToken: isValidToken(token) })).catch((error) => sendResponse({ token: "", hasToken: false, error: error instanceof Error ? error.message : String(error) }));
      return true;
    }
