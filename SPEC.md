...
- 自动批量写入必须先在内存中完成计划、唯一性检查和新鲜度复核，再备份、原子执行并可回滚；右键修改仍保留显式预览和应用动作；“KnowFlow：剪藏到这里”单独走 proposal 通道。
...
- `4.5.0` 增加文件夹自动编码、独立目录长编码索引、右键手动调整和白名单；`4.5.1` 将标签面板收窄为下拉选择和修改后路径预览，并精炼剪藏入口命名。4.4.0 的标签递归整理见 `docs/design/4.4-标签归类与递归整理.md`，本版本设计见 `docs/design/4.5-文件夹自动编码与白名单.md`。
