...
- 本地 Markdown 自动识别默认直接编码和改名；文件名与界面默认使用短编码，完整编码仍写入 YAML、同步事件和索引。异常项（保护路径、损坏 YAML、非法文档 ID、编码冲突、目标占用）单篇跳过并提示，用户可用右键纠错面板修正。
- 自动标签识别只能使用文档自身内容和协议封闭枚举；无足够信号时回退 `S` 并显示低置信度，不得凭目录名创造标签。
- 自动批量写入必须先在内存中完成计划、唯一性检查和新鲜度复核，再备份、原子执行并可回滚；右键纠错仍保留显式应用动作。
...
- `4.3.0` 是当前稳定版；自动编码与右键纠错设计见 `docs/design/4.3-自动编码与右键纠错.md`，发布与真机记录见 `PROGRESS.md` 和对应验收记录。
