...
function isInternalDriveUrl(value: string): boolean {
  try {
    const url = new URL(value);
    if (url.hostname === INTERNAL_API_HOST || url.hostname === INTERNAL_API_HOST_LARK) return true;
 ...
 */
export function extractTokenFromAuthcodeUrl(url: string): string | null {
  if (!url) return null;
  let u: URL;
  try {
    u = new URL(url);
  ...
  if (host !== INTERNAL_API_HOST && host !== INTERNAL_API_HOST_LARK && !API3_DRIVE_HOST_RE.test(host)) return null;
 ...
 */
export function rewriteImagesToFeishuProto(
  md: string,
  tokenMap: Set<string> | Map<string, string> = new Set(),
): string {
  // 匹配 ![alt](url) 形式的图片，url 是 internal-api 链接
  const imgRe = /!\[([^\]]*)\]\(([^)]+)\)/g;
  return md.replace(imgRe, (full, alt: string, url: string) => {
    const trimmed = url.trim().replace(/^<|>$/g, '');
    // 已经是 feishu:// 协议，跳过
    if (trimmed.startsWith(FEISHU_PROTO)) return full;
    ...
    if (isInternalDriveUrl(trimmed)) {
      ...
      if (token) return `![${alt}](${FEISHU_PROTO}${token})`;
    }
    // 普通外链或 base64，原样保留
    return full;
  });
...
function pickFromMap(tokenMap: Set<string> | Map<string, string>): string | null {
  if (tokenMap instanceof Map) return null;
  if (tokenMap.size === 0) return null;
  return tokenMap.values().next().value ?? null;
}

function pickExactToken(tokenMap: Set<string> | Map<string, string>, url: string): string | null {
  if (!(tokenMap instanceof Map)) return null;
 ...
 */
export function extractImgTokensFromXml(xml: string): string[] {
 ...
 */
export function extractImgTokenMapFromXml(xml: string): Map<string, string> {
  ...
  while ((m = imgRe.exec(xml)) !== null) {
    ...
    if (!src || !href || src.startsWith('http')) continue;
 ...
 */
export function extractFeishuImageTokens(md: string): string[] {
  const tokens = new Set<string>();
  const re = new RegExp(
    `!\\[[^\\]]*\\]\\(${FEISHU_PROTO.replace('/', '\\/')}([A-Za-z0-9]+)\\)`,
    'g',
  );
  let m: RegExpExecArray | null;
 ...
 */
export function alignFeishuImageTokens(localMd: string, remoteMd: string): string {
  const remoteTokens: string[] = [];
  remoteMd.replace(/!\[[^\]]*\]\(feishu:\/\/([A-Za-z0-9]+)\)/g, (_full, token: string) => {
    remoteTokens.push(token);
    return _full;
  });
  if (remoteTokens.length === 0) return localMd;

  let index = 0;
  return localMd.replace(/!\[([^\]]*)\]\(feishu:\/\/([A-Za-z0-9]+)\)/g, (full, alt: string) => {
 ...
 */
export function feishuProtoToXml(md: string): string {
  const re = /!\[([^\]]*)\]\(feishu:\/\/([A-Za-z0-9]+)\)/g;
  return md.replace(re, (_full, _alt: string, token: string) => {
