...
5. **整理发现**：新建或修改 Markdown 后是否生成待确认任务；全库扫描也可从 Ribbon 或命令面板发起
...
| 自动识别并整理全库文档… | 递归扫描、预览并一次确认整批编码 |
| 整理当前目录… | 当前目录直属层预览与批量编码 |
...
OB 插件：
  ...
  6. 返回落地路径与待确认 proposal（不静默编码）

用户可在 Ribbon 中执行“自动识别全库文档…”，由插件一次性预览并确认编码、YAML 和文件名变更。
