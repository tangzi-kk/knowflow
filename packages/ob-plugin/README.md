...
5. **自动识别并编码**：新建或修改 Markdown 后自动识别、分配完整编码并写入；界面与文件名默认显示短编码，重新归类时从文件树右键进入标签整理面板
6. **文件夹编码白名单**：相对 Vault 根目录填写，逗号或换行分隔；新建/重命名、插件启动扫描和手动整理都会跳过这些目录及子目录。固定结构同时保护 `🪧导引/`、`3️⃣附件文件/`，并让 `0️⃣输入/`、`1️⃣🗃知识池/` 的二级语义目录作为容器、容器自身不编码，从三级文件夹开始连续整理；`2️⃣输出/` 从二级文件夹开始编码
7. **知识库 space_id**：目录映射用；新安装留空，填写后再点“刷新映射”
...
| 剪藏到当前目录… | 输入飞书链接或 token，采集到指定目录并生成 proposal |
| 修改当前目录标签并整理… | 当前目录（含子目录）选择协议标签并进入事务预览 |
...
OB 插件：
  ...
  6. 返回落地路径与待确认 proposal；本地自动编码监听 Markdown 变化，并在插件启动时扫描固定目录容器

用户可在 Ribbon 中执行“自动识别并整理全库文档”，插件会自动提交安全项；插件启动时也会自动扫描 `0️⃣输入/`、`1️⃣🗃知识池/` 下的二级容器，按自然排序从 `01` 连续整理其直接子文件夹。需要把 `Q`、`Z` 等任一合法标签调整为其他合法标签时，在文件树右键选择“修改标签”，目录默认递归处理子目录中的 Markdown。文件夹右键的“剪藏到这里…”只负责采集，容器或子文件夹的结构调整使用“整理此容器编码…”。固定入口和二级容器自身不参与文件夹编码，短编码显示在目录名，长编码记录在 `.feishu-sync/目录编码索引.json`，不写 YAML。飞书剪藏 proposal 仍需确认。

标签修改面板只显示标签下拉框和修改后的路径预览；完整编码、重复编码和内部索引不会出现在普通界面。

编码约定：文件名和标签整理面板默认显示短编码（如 `S01.a1`）；完整编码（如 `26_0726_S_01_a1`）仍写入 YAML `编码`、同步事件和索引，作为唯一性与后端交互的事实值。
