...
6. **文件夹编码白名单**：相对 Vault 根目录填写，逗号或换行分隔；新建/重命名和手动文件夹编码都会跳过这些目录及子目录。固定结构同时保护 `🪧导引/`、`3️⃣附件文件/`，并让 `0️⃣输入/`、`1️⃣🗃知识池/` 从三级文件夹开始编码、`2️⃣输出/` 从二级文件夹开始编码
...
用户可在 Ribbon 中执行“自动识别并整理全库文档”，插件会自动提交安全项；需要把 `Q`、`Z` 等任一合法标签调整为其他合法标签时，在文件树右键选择“修改标签”，目录默认递归处理子目录中的 Markdown。文件夹右键的“KnowFlow：剪藏到这里…”只负责采集，结构调整使用“调整此文件夹结构编码…”。固定入口不参与文件夹编码，短编码显示在目录名，长编码记录在 `.feishu-sync/目录编码索引.json`，不写 YAML。飞书剪藏 proposal 仍需确认。
